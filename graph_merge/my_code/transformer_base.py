...
class TransformerModelBase(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, cfg, task):
        ...
        if cfg.share_all_embeddings:
            ...
            encoder_embed_tokens, en_graph_merger = cls.build_graph_embedding(
                cfg, src_dict, cfg.encoder.embed_dim, gnn_dropout, cfg.graph_path
            )
            decoder_embed_tokens, de_graph_merger = cls.build_graph_embedding(
                cfg, tgt_dict, cfg.decoder.embed_dim, gnn_dropout, cfg.graph_path
            )
        if cfg.offload_activations:
            cfg.checkpoint_activations = True  # offloading implies checkpointing
        encoder = cls.build_encoder(cfg, src_dict, encoder_embed_tokens)
        if cfg.share_all_embeddings:
            de_graph_merger = graph_merger
        decoder = cls.build_decoder(cfg, tgt_dict, decoder_embed_tokens, de_graph_merger)
        # if cfg.tie_graph_proj:
        #     print("build decoder with graph merger")
        #     decoder = cls.build_decoder(cfg, tgt_dict, decoder_embed_tokens, de_graph_merger)
        # else:
        #     print("build decoder without graph merge")
        #     decoder = cls.build_decoder(cfg, tgt_dict, decoder_embed_tokens, None)
    ...
    @classmethod
    def build_graph_embedding(cls, cfg, dictionary, embed_dim, gnn_dropout, graph_path, path=None):
        num_embeddings = len(dictionary)
        padding_idx = dictionary.pad()

        matrix_path = os.path.join(cfg.data, graph_path)
        print("load graph matrix: {}".format(graph_path))
        emb, graph_merger = MultiHopGraphEmbedding(num_embeddings, embed_dim, padding_idx, gnn_dropout, cfg.hop_num, matrix_path)
        # if provided, load from preloaded dictionaries
        if path:
            embed_dict = utils.parse_embedding(path)
            utils.load_embedding(embed_dict, dictionary, emb)
        return emb, graph_merger
...
class GraphMerge(nn.Module):
    def __init__(self, matrix_path, dim, device, type, gnn_dropout, hop_num):
        ...
        # self.graph_matrix = sparse_tensor.cuda()
        self.graph_matrix = sparse_tensor.to(device)

        # init linear_layers
        self.neighbor_layers = nn.ModuleList([])
        # self.neighbor_layers.extend([nn.Linear(dim, dim, bias=True, device=device, dtype=type).cuda()
        ...
        # self.self_layers.extend([nn.Linear(dim, dim, bias=True, device=device, dtype=type).cuda()
        #                          for _ in range(self.hop_num)])
    ...
    def forward(self, m):
        # m = m.cuda()
        self.graph_matrix = self.graph_matrix.to(m.dtype)
        ...
        for i in range(self.hop_num):
            ...
            H_neighbor = torch.matmul(self.graph_matrix, H_merge).to(m.dtype)  # [V, Dim]
            H_neighbor = self.neighbor_layers[i](H_neighbor)  # [V, Dim]
            H_self = self.self_layers[i](H_merge)  # [V, Dim]
            H_merge = H_neighbor + H_self  # [V, Dim]
            if i != self.hop_num - 1:
...
def MultiHopGraphEmbedding(num_embeddings, embedding_dim, padding_idx, gnn_dropout, hop_num, matrix_path):
    ...
    # init reparemeter
    print("hop num: {}".format(hop_num))
    print("gnn_dropout: {}".format(gnn_dropout))
    graph_merger = GraphMerge(matrix_path, m.weight.shape[1], m.weight.device, m.weight.dtype, gnn_dropout, hop_num)
    parametrize.register_parametrization(m, "weight", graph_merger)
    return m, graph_merger
