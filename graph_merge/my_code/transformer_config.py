...
class TransformerConfig(FairseqDataclass):
    ...
    gnn_dropout: float = field(
        default=0.1,
        metadata={"help": "gnn dropout ratio"},
    )
    graph_path: str = field(
        default="alignment_matrix.npz",
        metadata={
            "help": "detokenize before computing BLEU (e.g., 'moses'); required if using --eval-bleu; "
            "use 'space' to disable detokenization; see fairseq.data.encoders for other options"
        },
    )
    tie_graph_proj: bool = field(
        default=False,
        metadata={
            "help": "if set, ties the projection weights of reparameterized embedding and decoder output"
        },
    )
    hop_num: int = field(
        default=2,
        metadata={"help": "hop num"},
    )
